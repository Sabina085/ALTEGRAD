...
def sample_examples(docs,max_window_size,n_windows):
    ...
    for i,doc in enumerate(docs):
        ...
        # window_size = min(window_size, len(doc))
        for j in range(len(doc) - window_size):
            windows.append(get_windows(doc[j:j+window_size], window_size))            
    ...
    all_negs = choices(list(range(1,len(vocab) + 1)), neg_distr, k = n_negs*len(windows))
...
def compute_gradients(pos,negs,target,prodpos,prodnegs):
    factors_pos = 1/(np.exp(prodpos)+1) # (nr_pos, )
    factors_negs = 1/(np.exp(-prodnegs)+1) # (nr_negs, )
    ...
    term_pos = -Wc[pos, :].T * (np.reshape(factors_pos, (factors_pos.shape[0], 1))).T # term_pos (nr_pos, 30)
    term_negs = Wc[negs, :].T * (np.reshape(factors_negs, (factors_negs.shape[0], 1))).T # term_negs (nr_negs, 30)
    partial_target = np.sum(term_pos,axis=1) + np.sum(term_negs,axis=1) # partial target (30,)
...
if not train:
    ...
    print(my_cos_similarity("movie", "banana")) # 0.11
    print(my_cos_similarity("films", "movies")) # 0.9934
    print(my_cos_similarity("woman", "man")) # 0.985
    print(my_cos_similarity("sister", "brother")) # 0.9667
    print(my_cos_similarity("horror", "comedy")) # 0.9812
    print(my_cos_similarity("love", "mind")) # 0.976 -> Unexpected
