...
padding_idx = 0
oov_idx = 1
d = 30 # 100 # dimensionality of word embeddings
max_size = 60 # max allowed size of a document
nb_branches = 2
nb_filters = 50 # 100 
filter_sizes = [3,4]
drop_rate = 0.3 # 0.5 # amount of dropout regularization
batch_size = 64 # 32 # 8
nb_epochs = 2 # 1
my_optimizer = 'adam'
...
print('x_train.shape', np.asarray(x_train).shape) # x_train.shape (25000, 60)
...
doc_wv = Embedding(len(index_to_word) + 2, d, input_length = max_size) (doc_ints) ### fill the gap (add an Embedding layer) https://keras.io/layers/embeddings/ ###
...
model.compile(my_optimizer, loss='binary_crossentropy', metrics=['accuracy'])
...
doc_embs = get_doc_embedding([np.array(x_test[:n_plot]),0])[0] # 1000x100
...
get_region_embedding = K.function([model.layers[0]], [model.layers[3].output])
...
my_review = [word_to_index[elt] for elt in tokens] # 1 x 20
...
#print(reg_emb.shape) #18x50 (nb_filters = 50)
