...
gs = list()
for terms in abstracts_cleaned:
    gs.append(terms_to_graph(terms, 4))
...
for counter,g in enumerate(gs):
    ...
    main_core = []
    for term in core_numbers.keys():
        if core_numbers[term] == max_core:
            main_core.append(term)
    keywords['kc'].append(main_core)
...
for counter,vector in enumerate(vectors_list):
    ...
    for term, weight in terms_weights:
        if weight != 0:
            nonzero.append(tuple((term, weight)))
